use crate::ingress::handler::{Annotation, Key, Request};
use commonware_consensus::{
    simplex::types::Finalization,
    types::{Height, Round},
};
use commonware_cryptography::{Digest, certificate::Scheme as CertificateScheme};
...
pub(crate) struct Floor<S: CertificateScheme, C: Digest> {
    processed: ProcessedFloor,
    pending: Option<Finalization<S, C>>,
}

impl<S: CertificateScheme, C: Digest> Floor<S, C> {
    pub(crate) const fn resolved(height: Option<Height>, round: Round) -> Self {
        Self {
    ...
    pub(crate) const fn processed_height(&self) -> Height {
    ...
    pub(crate) fn await_anchor(&mut self, finalization: Finalization<S, C>) {
    ...
    pub(crate) fn fetch_targeted_if_permitted<R>(
        &self,
        resolver: &mut R,
        fetch: Request<C>,
    ...
    pub(crate) fn fetch_all_if_permitted<R>(
        &self,
        resolver: &mut R,
...
mod tests {
    use super::*;
    use crate::ingress::handler::Finalized;
    ...
    fn fetch_targeted_if_permitted_returns_denied_without_fetching() {
        ...
        assert_eq!(resolver.targeted(), vec![Key::Finalized { height: 6 }]);
    }
    ...
    fn fetch_all_if_permitted_filters_denied_requests() {
        ...
        let fetches = resolver.fetches();
        assert_eq!(fetches.len(), 2);
        assert!(matches!(fetches[0].key, Key::Finalized { height: 6 }));
        assert!(
    ...
    fn fetch_if_permitted_without_height_floor_allows_genesis_height() {
        ...
        assert!(matches!(
            fetches[0],
            Fetch {
                key: Key::Finalized { height: 0 },
                subscriber: Annotation::Finalized(Finalized::ByHeight {
                    height: subscriber_height
                }),
                ..
            } if subscriber_height == Height::zero()
        ));
    }
}
