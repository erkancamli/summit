...
#[pin_project]
pub(crate) struct PendingAck<B: Block, A: Acknowledgement> {
    pub(crate) height: Height,
    pub(crate) commitment: B::Digest,
    #[pin]
    pub(crate) receiver: A::Waiter,
}
...
pub(crate) struct PendingAcks<B: Block, A: Acknowledgement> {
...
impl<B: Block, A: Acknowledgement> PendingAcks<B, A> {
    ...
    pub(crate) const fn current(&mut self) -> &mut OptionFuture<PendingAck<B, A>> {
        &mut self.current
    }
    ...
    pub(crate) fn next_dispatch_height(&self, start_height: Height) -> Height {
        self.queue
            .back()
    ...
    pub(crate) fn enqueue(&mut self, ack: PendingAck<B, A>) {
    ...
    pub(crate) fn pop_ready(
        &mut self,
    ) -> Option<(Height, B::Digest, <A::Waiter as Future>::Output)> {
...
mod tests {
    ...
    fn enqueue_tracks_capacity_and_fifo_ready_order() {
        let mut pending = PendingAcks::<TestBlock, Exact>::new(2);
